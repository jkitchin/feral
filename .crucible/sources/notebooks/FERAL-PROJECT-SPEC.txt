...
│   └── Supernode detection and amalgamation
│   └── Assembly tree construction
│   └── MemoryPlan production (factor NNZ, contribution block sizes)
│
├── Memory Allocation     ← driven by MemoryPlan
│   └── FactorArena    (contiguous, preallocated)
│   └── ContribStack   (stack arena, LIFO in postorder)
│
├── Numeric Factorization
│   ├── Assembly (extend-add: scatter original entries + pop child contributions)
...
│
└── Configuration
    └── SupernodePolicy (None | Aggressive, based on n)
...
FERAL uses a **two-arena model** for all dynamic memory. No other heap allocation occurs during numeric factorization. This model is designed from the start for the single-thread case and extended cleanly to parallel in Phase 2.
...
pub struct MemoryPlan {
    /// Total NNZ in the L factor across all supernodes.
    /// Each supernode's slice is contiguous within factor_storage.
    pub factor_nnz: usize,

    /// For each node in postorder: the size (in f64s) of its contribution block.
    /// With no delayed pivoting, these are exact. With delays, treat as lower bounds.
    pub contrib_sizes: Vec<usize>,

    /// Peak contribution stack depth (sum of all live contribution blocks at
    /// the deepest point of the tree). Used to preallocate ContribStack.
...
One contiguous `Vec<f64>` preallocated to `factor_nnz` entries. Symbolic factorization assigns each supernode a fixed offset into this array. During numeric factorization, factor values are written in-place at their precomputed offsets. This array is permanent — it holds the factorization for the lifetime of the `Factors` struct.

#### ContribStack

One contiguous `Vec<f64>` preallocated to `peak_contrib_bytes` entries. The elimination tree is traversed in **postorder** (leaves first). At each node:

1. **Assemble** the frontal matrix: scatter original matrix entries + extend-add child contribution blocks (pop from stack top).
2. **Factor** the frontal with the dense kernel → accepted pivots written to `FactorArena`, contribution block produced.
3. **Push** the contribution block onto the stack.

The parent consumes (pops) the contribution block during its own assembly. In postorder traversal, a node's contribution is always the most recent push — the stack is truly LIFO and no bookkeeping beyond a `top` pointer is needed.

**Delayed pivoting:** When a pivot is delayed from node F to its parent, F's contribution block is larger than `MemoryPlan` predicted. Handle with a **slack factor**: preallocate `ContribStack` at 1.5× `peak_contrib_bytes`. If a push would overflow, fall back to heap allocation for that node's contribution and log a warning. This is rare on well-conditioned problems and is the correct behavior rather than a crash.

**Phase 2 (parallel):** Independent subtrees of the assembly tree can be factored concurrently. The simple LIFO stack breaks here — threads working different subtrees interleave pushes. Phase 2 replaces `ContribStack` with a **task-local stack per thread** (SSIDS model: each thread has its own arena, contribution blocks between threads use work-stealing). `FactorArena` remains shared but uses precomputed offsets so writes are non-overlapping and race-free. The `MemoryPlan` is unchanged — it is the interface between the two phases.
...
pub enum ZeroPivotAction {
    /// Add this value to the diagonal and re-use the pivot.
    /// For a 1×1 pivot d: new pivot = d + ε.
    /// For a 2×2 block [[a,b],[b,c]]: add ε to both diagonal entries — new block = [[a+ε, b], [b, c+ε]].
    /// Valid only when the matrix is known full-rank (near-zero pivot is numerical
    /// noise, not a true zero eigenvalue). Using Perturb on a genuinely rank-deficient
    /// matrix produces wrong inertia — see Section 2.4 for the full policy.
    Perturb(f64),
...
| Tolerant       | `0.1`                 | `zero_tol²`      | `Perturb(1e-8)`    | Near-singular, full-rank NLP late iter|
...
**Inertia policy when `ZeroPivotAction::Perturb` is used.** There is an inherent tension between "inertia is exact" and perturbation. The resolution:

- **`Perturb(ε)` is valid only when the matrix has full rank and the near-zero pivot is numerical noise**, not a true zero eigenvalue. In this case, the perturbed inertia is the same as the true inertia (the sign of the pivot is numerically determined; adding ε restores the correct sign). The reported inertia reflects the perturbed matrix, which equals the inertia of the original full-rank matrix.

- **`Perturb(ε)` on a genuinely rank-deficient matrix produces wrong inertia.** If the matrix truly has a zero eigenvalue, perturbation converts a zero to a positive contribution, reporting `zero = 0` when the correct value is `zero > 0`. FERAL does not detect this — the burden is on the caller to only use `Perturb` when they know the matrix is full-rank (e.g., the NLP is non-degenerate). This is documented in the `Perturb` variant's doc comment.
...
- **`Fail` returns `FeralError::NumericallyRankDeficient`** — the caller decides what to do. POUNCE interprets this as linearly dependent constraints and triggers its constraint reduction logic.

The `Tolerant` configuration in the table above (using `Perturb`) is valid for NLP late iterations where the Hessian is known to be non-singular but ill-conditioned. It is not valid for detecting rank-deficiency.

**Stage 2 / POUNCE integration:** When POUNCE exists and needs to adapt pivot behavior across IPM iterations (large barrier → tolerant, small barrier → tight), a `PivotStrategy` trait will be designed at that point, informed by what the multifrontal engine and POUNCE actually require. The adaptive capability — exploiting outer loop structure — remains a target novel contribution. Premature trait design is deferred until the interface can be grounded in real usage.
...
```rust
/// Symmetric matrix in packed lower-triangular column-major storage.
/// Entry (i, j) with i >= j is stored at index j*n - j*(j+1)/2 + i.
/// Size: n*(n+1)/2 f64 values.
pub struct SymmetricMatrix {
    pub n: usize,
    pub data: Vec<f64>,  // packed lower triangle, column-major
}
...
/// Stage 2 (multifrontal) uses a different type: SparseFactors stored in FactorArena.
pub struct Factors {
    pub n: usize,
    /// Unit lower triangular L in packed lower-triangular column-major storage.
    /// Same index formula as SymmetricMatrix: entry (i,j) with i >= j at index j*n - j*(j+1)/2 + i.
    /// The diagonal entries (i == j) are always 1.0 (unit lower triangular) and ARE stored
    /// explicitly, so l.len() == n*(n+1)/2. This avoids a special-case offset formula during
    /// forward/backward substitution. The diagonal cost is n extra f64s — negligible.
    pub l: Vec<f64>,
    /// D_bk blocks in pivot order. len() == number of pivot blocks, not n.
    /// Iterate sequentially: Block1x1 consumes one pivot position; Block2x2 consumes two
    /// consecutive pivot positions. The sum of block sizes equals n.
    /// Example: for n=4 with blocks [Block2x2, Block1x1, Block1x1], positions are 0–1, 2, 3.
    pub d_blocks: Vec<DBlock>,
    /// BK pivot permutation. Length n. Convention: perm[i] = j means original row j
    /// was moved to pivot position i during factorization.
...
pub enum DBlock {
    Block1x1(f64),
    Block2x2(f64, f64, f64),  // (a, b, c) = [[a,b],[b,c]]
}
```
...
### 2.6 Supernode Policy

```rust
pub enum SupernodePolicy {
    None,        // No amalgamation; for n < ~200
    Aggressive,  // Full detection and merge; for n > ~200
}
```

Controlled by problem size. The threshold is empirically tuned against the benchmark suite.
...
**Algorithm (Phase 1):** Infinity-norm diagonal equilibration. Compute a diagonal scaling matrix D such that each row i of D·A·D has ||row i||∞ ≈ 1:

```
d[i] = 1 / sqrt(max_j |A[i,j]|),  for i = 1..n
```
...
This is the same strategy used in rmumps (`scaling.rs`). It is sufficient for Phase 1 correctness.
...
- Stage 2 (sparse): always on; applied to the assembled frontal before calling the dense kernel.
...
**Phase 2 improvement:** MC64 (optimal matching-based scaling and ordering) produces better pivot quality on structurally difficult systems. This requires matching-algorithm infrastructure and is deferred to Phase 2. The MC64 reference is: Duff & Koster (1999/2001), HSL_MC64, available in SuiteSparse as `MATLAB/CSparse/cs_dmperm.m`. FERAL will implement a clean-room version from the algorithm description, not from HSL source.
...
**Step 4 in detail for 2×2 blocks:** For a 2×2 D block `[[a, b], [b, c]]`, solve `[[a,b],[b,c]] · [w1, w2]ᵀ = [z1, z2]ᵀ` directly: `det = a·c − b²`, `w1 = (c·z1 − b·z2) / det`, `w2 = (a·z2 − b·z1) / det`. If `|det|` is near zero, the block was near-singular — this should have been caught by `ZeroPivotAction` during factorization.
...
**Reference model:** LAPACK's `dgerfs` (double-precision iterative refinement for general systems). FERAL's version is simpler: no norm estimation, no condition number estimate, just 3 steps.
...
1. Scalar LDLᵀ with full Bunch-Kaufman pivoting (correct first)
2. Blocked LDLᵀ with cache-aware panel factorization
3. Optimized Schur complement update (symmetric rank-k)
4. SIMD micro-kernel for the inner loop (Rust `std::arch`)
5. Full micro-kernel architecture per BLIS (if profiling justifies)
...
| Median time ratio vs MUMPS | FERAL time / MUMPS time | < 2× |
...
│   │   ├── mod.rs
│   │   ├── matrix.rs            # SymmetricMatrix type (packed lower triangular, column-major)
│   │   ├── factor.rs            # Bunch-Kaufman LDLᵀ factorization
│   │   ├── solve.rs             # Full solve sequence per Section 2.9
│   │   └── equilibrate.rs       # Diagonal equilibration (Section 2.8)
...
│   # │   ├── frontal.rs         # Frontal matrix management, FactorArena, ContribStack
...
pub enum FeralError {
    ...
    /// A pivot became numerically zero after perturbation was attempted but
    /// the perturbation value was insufficient (perturbed value still ≤ zero_tol).
    /// Should not occur with well-chosen perturbation values.
    PerturbationFailed,
...
These four variants cover Stage 1. `NumericallyRankDeficient` is what POUNCE checks to detect degenerate constraints. Additional variants (e.g., `OutOfMemory`, `SymbolicAnalysisFailed`) will be added in Stage 2 with human review.
...
2. `matrix.data.len() != n*(n+1)/2` → error: `"packed data length {got} != expected {n*(n+1)/2} for n={n}"`
3. Any `f64::is_nan()` or `f64::is_infinite()` entry → error: `"matrix contains NaN or Inf at packed index {i}"`

For `solve()` and `solve_refined()`:

4. `rhs.len() != factors.n` → `FeralError::DimensionMismatch { expected: factors.n, got: rhs.len() }`

No other input validation is required. The caller is responsible for providing a lower-triangular matrix in the correct packed format. FERAL does not check structural symmetry (it only reads the lower triangle).
...
2. Implement the Stage 1 data structures: `SymmetricMatrix` (packed lower-triangular dense matrix), `Inertia`, `Factors`, `DBlock` (CSC sparse matrix is Phase 1b — do not implement in Session 1)
...
6. Wire `src/lib.rs` reexports. The lib.rs file should contain only `pub use` declarations — no implementation. Exact structure:
   ...
   pub use dense::factor::{factor, Factors, DBlock, BunchKaufmanParams, ZeroPivotAction};
   pub use dense::solve::{solve, solve_refined};
   pub use dense::matrix::SymmetricMatrix;
   pub use inertia::Inertia;
   pub use error::FeralError;
   ```
   This matches the file layout: `factor()`, `Factors`, `DBlock`, params types live in `dense/factor.rs`; `solve()` and `solve_refined()` live in `dense/solve.rs` (the "full solve sequence" file per Section 11). This lets callers write `use feral::{factor, solve, SymmetricMatrix, ...}` without knowing the internal module structure.
...
- The threshold parameter α = (1 + √17) / 8
- Why 1×1 and 2×2 blocks are necessary and sufficient (existence theorem)
- Inertia preservation via Sylvester's Law of Inertia
- What BLIS and faer do for their symmetric factorizations (code inspection)
- Numerical guard for exact zero pivots vs near-zero pivots
- How this scalar version will later extend to blocked factorization
...
- Dense LDLᵀ with full Bunch-Kaufman pivoting
- `BunchKaufmanParams` struct (no trait)
...
- AMD ordering — produces a **permutation vector** `perm: Vec<usize>` (not an elimination tree)
- Elimination tree construction from the permuted sparsity pattern of `P·A·Pᵀ` — separate step after AMD
- Postordering of the elimination tree (required for LIFO property of ContribStack)
- Column counts / fill estimation from the postordered tree (Liu's algorithm; reference: CHOLMOD `cholmod_rowcolcounts.c`)
- Symbolic factorization: supernode detection, `MemoryPlan` production
- `FactorArena` + `ContribStack` allocated from `MemoryPlan` before numeric work begins
- Numeric factorization: postorder traversal, extend-add assembly, dense kernel embedded
- Frontal matrix layout: **column-major, full symmetric (not packed)** — required for efficient Schur complement update (A_22 -= A_21 · D_bk⁻¹ · A_21ᵀ uses column-major BLAS-3 pattern)
- `DenseKernel` trait designed at this boundary (see Section 2.5) with human review (Section 5.4 trigger #9)
- No delayed pivoting initially — use `ZeroPivotAction::ForceAccept` as the Phase 1b default; this correctly reports inertia (including zeros for rank-deficient KKT matrices) and flags the factorization for iterative refinement. Do NOT use `Perturb` as the Phase 1b default: KKT matrices from interior point solvers can be genuinely rank-deficient (degenerate Hessians, redundant constraints), and `Perturb` would silently report wrong inertia. Delayed pivoting — which eliminates the need for this compromise — is added in Phase 2
...
- Full benchmark harness with KKT and SuiteSparse matrices
...
- Blocked dense LDLᵀ with cache-aware panel factorization
- SIMD micro-kernel for Schur complement update
- Shared-memory parallelism on assembly tree (Rayon)
- Optimized memory allocation and fill prediction
- METIS ordering option
- **Exit criterion:** Within 2× of MUMPS on medium set

### Phase 3: POUNCE Integration

- Replace MUMPS in ripopt with FERAL
- Adaptive pivot strategy driven by barrier parameter
- Iterative refinement for static pivoting recovery
