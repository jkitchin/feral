...
## Continuation — issue triage (#76, #77)

No `src/` changes in this continuation; bench is unchanged from the checkpoint
above (numeric code byte-identical). Two diagnostic-only investigations:

### #76 — near-singular KKT inertia (CLOSED, not planned)
Author reframed the issue: reference was MUMPS 5.6.2 (via IPOPT 3.14.19), not
MA57; dense `eigvalsh` shows cond ≈ 1.25e29, numerically singular, ~23
eigenvalues at |λ|≈1e-14…1e-12. Negative counts dense 491 / feral 489 / MUMPS
≈488 are all arbitrary cuts in the zero cluster — feral's 489 is not "more
wrong" than 488. Declined the pivot-path rank-detection change (a single
isolated ~0 eigenvalue is provably invisible to BK pivot magnitude — Higham
2002 Ch.11; and any threshold catching the 1.2e-12 pivot risks the inertia
gate, the trade `inertia-near-singular-certification.md` already rejected).
Commented + closed not-planned. Matrix retained as a potential
singular-detection fixture.

### #77 — dense n=1000 sensors "unsolvable" (commented, LEFT OPEN)
Reported 45-55s first factor / 7-9s refactor, 6× δ_w cascade blows 300s.
**Does not reproduce.** feral on the dumped matrix (inertia (388,612,0), 612
negs matches): first factor **0.05s release** / 2.69s debug; cached refactor
0.03s / 2.41s; the full 6-factor cascade **0.286s total**. Every ordering ×
scaling 0.044-0.080s. All three hypotheses refuted — symbolic 0.02s; kernel
≈11 Gflop/s (within 2-4× of LAPACK dsytrf); n=1000 > D.3 gate (N_MAX=128) so it
takes the multifrontal path, which already amalgamates the complete graph to 1
supernode. The 45-55s is pounce-side (debug `maturin` ext, first-touch AD
assembly of the dense 1000×1000 Hessian, machine throttling) bundled into the
"factorization" number; issue's own figures are inconsistent (45-55 + 5×8 = 85s
> "6 factors in 60s cap"). Left open pending an isolated, release-built
`feral::factor()` timing on pounce's *live* matrix.

Throwaway probes used (`probe_issue76_warmstate`, `probe_issue77_densefront`,
`probe_issue77_sweep`) were removed after use — not committed.
