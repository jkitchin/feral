...
`cargo test --release`: 433 lib + all integration targets, 0 failed.
(Corrected after review: this line originally read 431. `assemble-context.sh` ran before the last test additions landed, and the stale count was copied into the checkpoint and the PR body.)
