...
/// Deterministic dense-coupling-column symmetric quasidefinite KKT used as an
/// issue-#80 regression fixture (the MC64 dense-column / near-tree cost class).
///
/// Returns `(n, lower_triangle_triplets, rhs, inertia)` where each triplet is
/// `(row, col, val)` with `row >= col`. The matrix is a symmetric permutation
/// of `K = [[D1, A^T], [A, -D2]]` with `D1` (n_var) and `D2` (n_con) diagonal
/// SPD, so by the Vanderbei (1995) symmetric-quasidefinite theorem its inertia
/// is exactly `(n_var positive, n_con negative, 0 zero)` for any `A` — an
/// external oracle independent of feral.
///
/// The dense constraint is placed at index 0 so column 0 carries a sub-diagonal
/// entry to every variable: a genuine dense column (degree `n_var + 1`) in the
/// stored lower-triangle CSC, visible to the symmetric MC64 matching. See
/// `dev/research/densecol-regression-fixture-2026-06-06.md`.
///
/// `(n, lower-triangle triplets, rhs, inertia)`.
type DenseColFixture = (usize, Vec<(usize, usize, f64)>, Vec<f64>, Inertia);

fn gen_densecol_kkt(n_var: usize, n_con: usize) -> DenseColFixture {
    let n = n_var + n_con;
    let mut t: Vec<(usize, usize, f64)> = Vec::new();

    // index 0: dense constraint. Negative diagonal (part of -D2).
    t.push((0, 0, -(1.0 + 0.1 * (n_var % 4) as f64)));
    // Dense coupling: every variable v in 1..=n_var couples to constraint 0.
    for v in 1..=n_var {
        let sign = if v % 2 == 0 { 1.0 } else { -1.0 };
        t.push((v, 0, sign * (0.5 + 0.05 * (v % 11) as f64)));
    }
    // indices 1..=n_var: variables. Positive diagonal (D1, SPD).
    for v in 1..=n_var {
        t.push((v, v, 1.0 + 0.1 * (v % 7) as f64));
    }
    // indices n_var+1..n: remaining n_con-1 constraints. Negative diagonal,
    // each banded to two lower-index variables (row > col, kept in range).
    for k in 0..n_con.saturating_sub(1) {
        let idx = n_var + 1 + k;
        t.push((idx, idx, -(1.0 + 0.1 * (k % 4) as f64)));
        let p1 = 1 + (k % n_var.max(1));
        let p2 = 1 + ((k + 1) % n_var.max(1));
        t.push((idx, p1, 1.0));
        if p2 != p1 {
            t.push((idx, p2, 0.75));
        }
    ...
    let rhs: Vec<f64> = (0..n).map(|i| 0.1 * (i % 13 + 1) as f64).collect();
    let inertia = Inertia {
        positive: n_var,
        negative: n_con,
        zero: 0,
...
/// Write a symmetric `.mtx` (Matrix Market coordinate, lower triangle) for a
/// fixture matrix. `triplets` must already be lower-triangle (`row >= col`).
fn write_mtx_symmetric(
    path: &Path,
    n: usize,
    ...
    writeln!(
        w,
        "% Synthetic issue-#80 dense-coupling-column quasidefinite KKT."
    )?;
    writeln!(
        w,
        "% Regenerated by bench (not committed). Inertia oracle: Vanderbei (1995) SQD."
    )?;
...
/// Write a KKT sidecar JSON matching the `KktSidecar` schema (`src/io/sidecar.rs`).
fn write_densecol_sidecar(
    path: &Path,
    problem_name: &str,
    n_var: usize,
    n_con: usize,
    ...
    let mut w = BufWriter::new(File::create(path)?);
    let rhs_json: Vec<String> = rhs.iter().map(|x| format!("{:.17e}", x)).collect();
    writeln!(
        w,
        "{{\"problem_name\":\"{}\",\"iteration\":0,\"n\":{},\"m\":{},\
         \"rhs\":[{}],\
...
/// Regenerate the issue-#80 dense-column regression fixtures on disk and return
/// the directory holding them. Deterministic (same content every run); written
/// even though `data/matrices/` is gitignored, so the dense-column path is
/// exercised by `cargo run --bin bench` on any clone, with no committed matrix
/// or result. See `dev/research/densecol-regression-fixture-2026-06-06.md`.
fn ensure_densecol_regression_fixtures() -> Result<PathBuf, std::io::Error> {
    let root = PathBuf::from("data/matrices/synthetic-regression");
    // (n_var, n_con) ladder: small/fast for the default run; max_col_degree
    // = n_var+1 exercises the dense-column MC64 path under FERAL_SCALING=mc64.
    let cases = [(300usize, 3usize), (1000usize, 4usize)];
    for (n_var, n_con) in cases {
        let stem = format!("densecol_kkt_{}", n_var);
        let dir = root.join(&stem);
        std::fs::create_dir_all(&dir)?;
        let base = format!("{}_0000", stem);
        let (n, triplets, rhs, inertia) = gen_densecol_kkt(n_var, n_con);
        write_mtx_symmetric(&dir.join(format!("{}.mtx", base)), n, &triplets)?;
        write_densecol_sidecar(
...
fn main() {
    ...
    // Issue-#80 dense-column regression fixtures. Regenerated on disk every
    // run (gitignored, no committed matrix/result) and prepended so they run
    // first and unconditionally — even with no corpus present. They flow
    // through the sparse loop like any KKT: inertia is checked against the
    // Vanderbei (1995) SQD oracle in the sidecar, residual against n·eps·1e6.
    // See dev/research/densecol-regression-fixture-2026-06-06.md.
    match ensure_densecol_regression_fixtures() {
        Ok(dir) => {
            let part = load_kkt_dir(&dir);
            if !part.is_empty() {
                println!(
                    "Generated {} issue-#80 dense-column regression fixture(s) in {}",
                    part.len(),
                    dir.display()
                );
                let mut combined = part;
                combined.append(&mut kkt_entries);
                kkt_entries = combined;
            }
        }
        Err(e) => eprintln!("warning: could not generate #80 regression fixtures: {e}"),
    }
...
mod tests {
    ...
    /// matrix must store a genuine dense column so it exercises the dense-column
    /// MC64 path. See dev/research/densecol-regression-fixture-2026-06-06.md.
    #[test]
    fn densecol_kkt_is_dense_column_sqd() {
        let (n_var, n_con) = (50usize, 3usize);
        let (n, triplets, rhs, inertia) = gen_densecol_kkt(n_var, n_con);

        assert_eq!(n, n_var + n_con);
        assert_eq!(rhs.len(), n);
        assert!(rhs.iter().all(|x| x.is_finite()));

        // External oracle: symmetric quasidefinite inertia (n_var, n_con, 0).
        assert_eq!(
            inertia,
            Inertia {
                positive: n_var,
        ...
        // Lower-triangle storage only (row >= col).
        assert!(triplets.iter().all(|&(r, c, _)| r >= c));

        // Column 0 is the dense coupling column: a sub-diagonal entry to every
        // variable plus its own diagonal => degree n_var + 1.
        let col0_degree = triplets.iter().filter(|&&(_, c, _)| c == 0).count();
        assert_eq!(
            col0_degree,
            n_var + 1,
            "dense coupling column degree {col0_degree} != n_var+1 ({})",
            n_var + 1
        );

        // The (1,1) variable block has strictly positive diagonals (SPD), the
        // constraint diagonals are strictly negative (-D2) — the SQD shape the
        // inertia oracle relies on.
        for &(r, c, v) in &triplets {
            if r == c {
                if (1..=n_var).contains(&r) {
                    assert!(v > 0.0, "variable diagonal {r} not positive: {v}");
                } else {
                    assert!(v < 0.0, "constraint diagonal {r} not negative: {v}");
                }
            }
        }
    }
