//! Profile feral's per-iter factor cost on robot_1600.
//!
//! Context: live ipopt-feral trace shows MC64 is only 2.6 % of
//! wall on robot_1600 (350 ms of 13.7 s total) yet the system-level
//! MA57 gap is 32×. The factor cost therefore lives in numeric +
//! ordering + symbolic, not scaling. This probe localises where.
//!
//! Loads the 7 dumped robot_1600 KKTs and runs them through:
//! 1. Per-factor sequential profiler on the first 3 (bucket
//!    distribution + top-N hottest supernodes).
//! 2. Warm sequence (one Solver, sequential, no profiler) for
//!    total-wall measurement vs the live ipopt-feral number.
//!
//! Run:
//!   cargo run --release --bin probe_robot_profile
...
fn main() {
    let csc_list: Vec<(usize, feral::CscMatrix)> = (0..7)
        .filter_map(|i| {
            let p = format!(
                "data/matrices/kkt-mittelmann/robot_1600/robot_1600_{:04}.mtx",
                i
            );
            let path = Path::new(&p);
            if !path.exists() {
    ...
    if csc_list.is_empty() {
        eprintln!("no robot_1600 KKTs found");
        return;
    }
    let (i0, csc0) = &csc_list[0];
    println!(
        "robot_1600 iter {} (n={}, nnz={})",
        i0,
        csc0.n,
        csc0.row_idx.len()
    );
