//! Issue #10 Phase 2: TppMethod::Plain vs TppMethod::Maxfromm A/B on
//! the 1D-banded Mittelmann panel that #33 identified as 97% scalar
//! 1×1 bound. SLB cannot move this number (#33 A/B); MAXFROMM should
//! cut the per-pivot AMAX scan to roughly zero.
//!
//! Reports min-of-N times, per-matrix speedup, per-family geomean, and
//! the panel decision per the issue-10 success criterion: median
//! speedup ≥ 2.0× AND no per-matrix slowdown > +5%.
//!
//! Usage: `cargo run --release --bin diag_clnlbeam_maxfromm`

use std::path::Path;
use std::time::Instant;

use feral::dense::factor::TppMethod;
...
fn params_with(tpp: TppMethod) -> NumericParams {
    NumericParams {
...
fn process_family(family: &str) -> (Vec<f64>, usize, usize) {
    ...
    println!(
        "{:>20}  {:>7}  {:>10}  {:>10}  {:>7}",
        "label", "snodes", "plain_us", "mfm_us", "speedup"
    );
    ...
    for (label, path) in &matrices {
        ...
        let plain = bench_path(&csc, &params_with(TppMethod::Plain), &sym);
        let mfm = bench_path(&csc, &params_with(TppMethod::Maxfromm), &sym);
        if plain == 0 || mfm == 0 {
            println!("{label:>20}  factor FAILED in either config");
            continue;
        }
        let speedup = plain as f64 / mfm as f64;
...
fn main() {
    println!("=== Issue #10 TppMethod::Plain vs Maxfromm A/B (1D-banded Mittelmann) ===");
    println!("min-of-{N_REPEAT} per config; warm-up uncounted; speedup = plain/mfm");
    ...
    if !all_speedups.is_empty() {
        ...
        println!(
            "\n#10 success criterion (median >= 2.00x AND max regression <= +5%): {}",
            if med >= 2.0 && min >= 0.95 {
                "PASS — recommend flipping default"
            } else if med >= 2.0 {
                "MIXED — median passes but worst case regresses > 5%"
            } else if med >= 1.10 {
                "PARTIAL — meaningful win but below the 2x prediction"
            } else {
                "FAIL — signal within noise"
            }
        );
    }
}
