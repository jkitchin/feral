...
#
# The six ordering crates (feral-amd, …) version independently and are
# NOT touched here.
#
# Usage:
...
do_check() {
    ...
    if [ "$versions_ok" -eq 0 ]; then
        ...
        return 1
    elif [ "$changelog_ok" -eq 0 ]; then
        echo "RESULT: all six version strings agree on $canonical, but"
        echo "        CHANGELOG.md still needs a [$canonical] section."
        return 1
    else
        echo "RESULT: all six version strings agree on $canonical."
        return 0
    fi
}
