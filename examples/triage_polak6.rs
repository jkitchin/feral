...
fn main() {
    ...
    println!("\nDiagonal range:    [{:.2e}, {:.2e}]  ratio={:.1e}",
        min_diag, max_diag, max_diag / min_diag);
    ...
    for &r in &rhs {
        let a = r.abs();
        if a > max_rhs { max_rhs = a; }
        if a > 0.0 && a < min_rhs { min_rhs = a; }
    }
    println!("RHS range:         [{:.2e}, {:.2e}]  ratio={:.1e}",
        min_rhs, max_rhs, max_rhs / min_rhs);
    ...
    println!("\nplain solve  rel_res = {:.3e}", rel_residual(&csc, &dx, &rhs));
    let dxr = solve_refined(&dense_mat, &dfac, &rhs).expect("dense solve_refined");
    println!("refined solve rel_res = {:.3e}", rel_residual(&csc, &dxr, &rhs));
    ...
    println!("inertia: {} ({})", dinertia2,
        if dinertia2 == expected { "MATCH" } else { "MISMATCH" });
    ...
    println!("inertia: {} ({})", dinertia3,
        if dinertia3 == expected { "MATCH" } else { "MISMATCH" });
