...
fn main() {
    let path = std::env::args().nth(1).expect("usage: feral_time <kkt.bin>");
    ...
    for _ in 0..nnz { ia.push(read_i64(&mut f) as usize); }
    for _ in 0..nnz { ja.push(read_i64(&mut f) as usize); }
    for _ in 0..nnz { vals.push(read_f64(&mut f)); }
    ...
    for k in 0..nnz {
        let i = ia[k] - 1;
        let j = ja[k] - 1;
        if i >= j { rows_0.push(i); cols_0.push(j); }
        else      { rows_0.push(j); cols_0.push(i); }
    }
